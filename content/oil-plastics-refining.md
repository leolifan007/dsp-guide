...
{{< callout warning >}}
**Common mistake:** Building extractors, connecting a single belt to one refinery, and wondering why production crawls. You need volume 鈥?multiple extractors feeding a central refinery bank.
{{< /callout >}}

> **Finding oil:** You won't find sea planets early. Explore with your Vessel or use the galaxy map to scout 鈥?oil seas are on wet or oceanic planets. See our [Titanium Transport Guide](/titanium-transport-guide/) for how to set up interplanetary mining outposts.
...
This is your default recipe. Set it and forget it 鈥?as long as your hydrogen is being consumed downstream.
...
Set up a dedicated hydrogen bus: Tank 鈫?refineries (hydrogen output) 鈫?Hydrogen Fuel Plants.
...
That hydrogen output is itself the fuel for the fuel plants. It's a cycle 鈥?and once balanced, it runs cleanly.
...
Scale Plastics by adding more refinery units 鈥?not by pushing a single refinery faster.
...
The bottleneck is *volume*. One Rocket Fuel Refinery consumes 2 units of Refined Oil per second 鈥?roughly double what a single oil extractor provides.

**Scale:** Aim for 2鈥? Refineries dedicated to Rocket Fuel for every 5鈥? oil extractors.

### Fuel Plant Power on Rocket Fuel

Rocket Fuel can also be burned in Fuel Plants 鈥?at higher energy density than hydrogen:
...
1. **Oil extractors** on a dedicated patch 鈫?belt to central refinery bay
2. **Refineries** in the bay, each set to appropriate recipe
3. **Tanks** on hydrogen output lines 鈥?buffer and pressure valve
4. **Hydrogen Fuel Plants** fed from tank 鈫?stabilize hydrogen demand
...
{{< callout tip >}}
**Pro Tip:** Keep refining isolated on its own planet or dedicated area. Refineries are heavy consumers 鈥?mixing them into your main production bus makes balancing painful.
{{< /callout >}}
...
**Extractor 鈫?Refinery ratio:** 1 extractor feeds ~1 refinery comfortably. Scale with multiples.


Oil isn't complicated 鈥?it's just demanding. One Refinery bank solving for hydrogen keeps everything downstream clean. Run hydrogen into Fuel Plants, use the refined oil for Plastics, and scale with parallel refineries rather than faster ones. The moment your refinery tanks are backed up, check your hydrogen consumer count first. More often than not, that's the bottleneck.
